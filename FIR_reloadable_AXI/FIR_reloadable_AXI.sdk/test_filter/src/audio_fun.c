...
int AudioInitialize(u16 timerID,  u16 iicID, u32 i2sAddr)
{
	...
	Status = AudioRegSet(&Iic, 15, 0b000000000); //Perform Reset
		...
		Status |= AudioRegSet(&Iic, 0, 0b000000101); //001101  13/64 bajar mas volumen
		Status |= AudioRegSet(&Iic, 1, 0b000000101);  //001101  13/64 bajar mas volumen
		Status |= AudioRegSet(&Iic, 2, 0b001101110); //  100/127
		Status |= AudioRegSet(&Iic, 3, 0b001101110); //sssss
		Status |= AudioRegSet(&Iic, 4, 0b000010000);//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxaqui
		Status |= AudioRegSet(&Iic, 5, 0b000000100);
		Status |= AudioRegSet(&Iic, 7, 0b000001010); //Changed so Word length is 24
		Status |= AudioRegSet(&Iic, 8, 0b000100000); //Changed so no CLKDIV2, sampl rate  44.1kHz

		//Status |= AudioRegSet(&Iic, 16, 0b110000001);//pga gain //target level 2/15, last that worked
		//Status |= AudioRegSet(&Iic, 17, 0b000000000);//pga gain //target level 2/15, last that worked

		//setting noise gate,
			//	Status |= AudioRegSet(&Iic, 18, 0b011100101); //
