...
int AudioInitialize(u16 timerID,  u16 iicID, u32 i2sAddr)
{
	...
	//setting de-emphasis to 44.1kHz and not enabling high pass filter
	Status |= AudioRegSet(&Iic, 5, 0b000000110); // cont'd
	//Status |= AudioRegSet(&Iic, 5, 0b000000000); // cont'd
	Status |= AudioRegSet(&Iic, 7, 0b000001010); // Word length is 24
	//set sampling freq to 44.1 000100000
	Status |= AudioRegSet(&Iic, 8, 000100000); // No CLKDIV2

	//setting up ALC in both channels, MAXGAIN 30dB
	Status |= AudioRegSet(&Iic, 16, 0b110001011);
	//setting noise gate,
	Status |= AudioRegSet(&Iic, 18, 0b0110010001); //
